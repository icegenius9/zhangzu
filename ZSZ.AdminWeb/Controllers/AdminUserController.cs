...
namespace ZSZ.AdminWeb.Controllers
{
    public class AdminUserController : Controller
    {
        ...
        {
            //AOP。AuthorizeFilter(controller(方法)执行之前用于权限控制)
            //IActionFilter(controller(方法)执行之前执行之后都会执行)
            //IResultFilter(每一个ActionResult执行的时候会被调用)
        ...
        {   //取得所有的城市
            ...
            //在cities(insert只能插入list中)最前面插入一个总部
